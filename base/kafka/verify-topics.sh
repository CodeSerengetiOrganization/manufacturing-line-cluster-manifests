...
KAFKA_BROKER_POD=$(kubectl get pods -n "${NAMESPACE}" -l strimzi.io/cluster=kafka -o name | grep broker | head -n 1 | cut -d/ -f2)
...
echo ""
echo "Verifying manufacturing-results-topic retention settings (expected: 172800000 ms = 48 hours):"
echo "---"
kubectl exec "$KAFKA_BROKER_POD" -n "${NAMESPACE}" -c kafka -- \
